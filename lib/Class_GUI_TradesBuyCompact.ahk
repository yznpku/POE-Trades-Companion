Class GUI_TradesBuyCompact {
	...
	HotBarButton(btnType) {
		...
		GUI_TradesBuyCompact_LeagueHelpMenu:
			file := A_ThisMenuItem="Betrayal"?"Betrayal.png"
				: A_ThisMenuItem="Delve"?"Delve.png"
				: A_ThisMenuItem="Essence"?"Essence.png"
				: A_ThisMenuItem="Incursion"?"Incursion.png"
				: ""
			if (!file)
				return
			...
			hBitMap := Gdip_CreateResizedHBITMAP_FromFile(PROGRAM.CHEATSHEETS_FOLDER "\" file, A_ScreenWidth*0.90, A_ScreenHeight*0.80, keepRatio:=True)
			SetImage(GuiCheatSheet_Controls.hIMG_CheatSheet, hBitmap)

			__f := GUI_TradesBuyCompact.CheatSheetRemove.bind(GUI_TradesBuyCompact)
